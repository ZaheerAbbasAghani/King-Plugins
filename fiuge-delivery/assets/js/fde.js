jQuery(document).ready(function(){
	...
	var val  = jQuery(this).val();
		if(val == "Pickup"){
			...
			if(response.status == 1){
				...
				jQuery("form.woocommerce-checkout").css("opacity","1");
				jQuery("form.woocommerce-checkout").attr("disabled",false);
				jQuery(".delivery_option").prop("checked", false);
			}
	...
	jQuery("#billing_postcode").on("keydown", function(){
		jQuery(".delivery_option").prop("checked", false);
	});

	jQuery("#billing_city").on("keydown", function(){
		jQuery(".delivery_option").prop("checked", false);
	});

	jQuery("#billing_address_1").on("keydown", function(){
		jQuery(".delivery_option").prop("checked", false);
	});
