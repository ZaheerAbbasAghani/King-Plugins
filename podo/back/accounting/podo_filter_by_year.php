...
add_action('wp_ajax_nopriv_podo_filter_by_year','podo_filter_by_year' );
add_action( 'wp_ajax_podo_filter_by_year', 'podo_filter_by_year' );
function podo_filter_by_year() {
...
$query3 ="SELECT created_at,sum(price) as revenue FROM $table_name WHERE YEAR(created_at)=$y AND status='pending' GROUP BY date(created_at) ORDER BY created_at DESC LIMIT 10";
