	...
	<form method="post" action id="update_new_customer">
		...
		<div class="column">
			<label>Geschlecht:
				...
				';
if($results[0]->gender == "Mann") {
	$info .='<option value="Mann" selected>Mann</option>';
	$info .='<option value="Frau">Frau</option>';
}
if($results[0]->gender == "Frau") {
	$info .='<option value="Mann">Mann</option>';
...
$info .='</select>
		...
		<div class="column_full">
			<label>Adresse:
				<textarea placeholder="Geben Sie Ihre Adresse ein" name="address" id="address" row="5" cols="5">'.$results[0]->address.'</textarea>
			</label>

			<label>Datum der ersten Behandlung:
				<input type="date" name="fad" id="fad" placeholder="Enter your first appointment date" value="'.$results[0]->fad.'">
			</label>
		...
		<div class="column_full">
			<label>Wichtige Informationen:
				<textarea placeholder="Enter important notes" name="important_notes" id="important_notes" row="5" cols="5" placeholder="Müssen wir etwas Wichtiges wissen?">'.$results[0]->important_notes.'</textarea>
			</label>
		</div>
