...

{
    ...
    $myRes = explode('-', $dp_day_of_year_number );

 //   echo $dp_day_of_year_number;


?>

    <input type="text" name="dp_day_of_year_number" value="<?php echo $myRes[0]; ?>" placeholder="Day of Year Number" style="width: 100%;border: 1px solid #ddd;padding: 6px;font-size: 16px;" id="dp_day_of_year_number" data-id="<?php echo $post->ID; ?>">
...

{
    ...
    $posts_with_meta = get_posts( array(
        ...
        'meta_value' => $_POST["dp_day_of_year_number"].'-'.$lan,
    ...
    if ( count( $posts_with_meta ) ) {
        ...
        if($posts_with_meta[0] != $post->ID){
            ...
            update_post_meta($post->ID, "dp_day_of_year_number", $_POST["dp_day_of_year_number"].'-'.$lan);
        }
