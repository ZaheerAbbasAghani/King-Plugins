...
foreach ( $screens as $screen ) {  add_meta_box(  'EventInformation',  'Event Information',  'bpem_show_custom_meta_box',  $screen,  'normal',  'high'  );
...
echo '<p> Event Belong to <b>'.bp_get_group_name(groups_get_group($evn_group)).'</b> Group</p>';echo '<p> GDPR Agreement <b style="text-transform:capitalize">'.   $evn_group = get_post_meta( $post->ID, 'gdpr_compliant', true ).'</b> </p>';
echo "</div>";
...
if ( ! isset( $_POST['evn_location_nonce'] ) &&   ! isset( $_POST['evn_startDate_nonce'] )  &&    ! isset( $_POST['evn_startTime_nonce'] ) &&   ! isset( $_POST['evn_endDate_nonce'] )  &&    ! isset( $_POST['evn_endTime_nonce'] ) &&  ! isset( $_POST['evn_group'] ))  

{
  return;
}

if (! wp_verify_nonce( $_POST['evn_location_nonce'], 'evn_location' ) &&   ! wp_verify_nonce( $_POST['evn_startDate_nonce'], 'evn_startDate' ) &&   ! wp_verify_nonce( $_POST['evn_startTime_nonce'], 'evn_startTime' ) &&   ! wp_verify_nonce( $_POST['evn_endDate_nonce'], 'evn_endDate' ) &&   ! wp_verify_nonce( $_POST['evn_endTime_nonce'], 'evn_endTime' ) &&  ! wp_verify_nonce( $_POST['evn_group_nonce'], 'evn_group' ) ) {
  return;
}
...
if ( ! isset( $_POST['evn_location'] ) &&    ! isset( $_POST['evn_startDate'] ) &&    ! isset( $_POST['evn_startTime'] ) &&    ! isset( $_POST['evn_endDate'] ) &&   ! isset( $_POST['evn_endTime'] ) &&   ! isset( $_POST['evn_group'] )  ) {
  return;
}
...
$columns = array(  'cb' => '&lt;input type="checkbox" />',  'title' => __( 'Event' ),  'groupname' => __( 'Group Name' ),  'startdate' => __( 'Start Date' ),  'enddate'  => __( 'End Date' ),  'date' => __( 'Publish' )
...
function custom_book_column( $column, $post_id ) {
	switch ( $column ) {
	  case 'groupname' :  $evn_group = get_post_meta( $post_id, 'evn_group', true );  $GN = bp_get_group_name(groups_get_group($evn_group));  echo '<p>'.__($GN).'</p>';  break;
	  case 'startdate' :  $startdate = get_post_meta( $post_id, 'evn_startDate', true );  echo '<p>'.__( $startdate).'</p>';  break;
	  case 'enddate' :  $enddate = get_post_meta( $post_id, 'evn_endDate', true );  echo '<p>'.__($enddate).'</p>';  break;
