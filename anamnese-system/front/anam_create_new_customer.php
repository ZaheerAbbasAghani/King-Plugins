...
function anam_create_new_customer() {
	...
	$wpdb->query("INSERT INTO $table_name (first_name, last_name, email_address,gender,birth_date,mobile_no,job,city,zipcode,fad,address,doctor_name,diagnosis,phone_of_doctor,drugs,insurance_company,important_notes,doctor_id) VALUES ('$first_name', '$last_name', '$email_address','$gender', '$birth_date', '$mobile_no', '$job', '$city', '$zipcode', '$fad', '$address', '$doctor_name', '$diagnosis', '$phone_of_doctor', '$drugs', '$insurance_company', '$important_notes','$doctor_id')");
	...
	$response = array("message" => "New Customer Created Successfully.", "status" => 1);
	echo wp_send_json($response);
	}else{
		//echo "Customer already exists!";	
		$response = array("message" => "Customer already exists!", "status" => 0);
		echo wp_send_json($response);
	}
