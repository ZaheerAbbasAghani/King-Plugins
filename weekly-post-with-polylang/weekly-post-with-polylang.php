<?php
/*
Plugin Name: Weekly Post With Polylang
...
Text Domain: weekly-post
Domain Path: /languages
*/

defined("ABSPATH") or die("No direct access!");
class WeeklyPostWithPolylang {
...
function dp_start_from_here() {
	...
	require_once plugin_dir_path(__FILE__) . 'front/dp_save_custom_field.php';
...
// Show Post on top if week exists
function make_the_post_sticky(){
	$week = ltrim(strftime('%V'),"0");

	$val = $week.'-'.pll_current_language();
...
if (class_exists('WeeklyPostWithPolylang')) {
	$obj = new WeeklyPostWithPolylang();
